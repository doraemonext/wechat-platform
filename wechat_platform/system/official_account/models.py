...
class OfficialAccountManager(models.Manager):
    def add(self, level, name, email, original, wechat, introduction=None, address=None,
        ...
        logger_official_account.debug('[OfficialAccount] New model created with iden %s [Model] %s' % (official_account.iden, official_account.__dict__))
...
class OfficialAccount(models.Model):
    ...
    def get_cache_access_token(self, force_update=False):
        ...
        if not self.appid or not self.appsecret:
            logger_official_account.warning('[OfficialAccount] Lack of appid or appsecret in the official account [Model] %s' % self.__dict__)
        ...
        try:
            ...
            logger_official_account.error('[OfficialAccount] AppID or AppSecret is incorrect [Detail] %s [Model] %s' % (e, self.__dict__))
        ...
        if self.cache_access_token != access_token_dict['access_token'] or self.cache_access_token_expires_at != access_token_dict['access_token_expires_at']:
            logger_official_account.debug('[OfficialAccount] Ready to update the model [Model] %s' % self.__dict__)
            ...
            self.save()
            logger_official_account.debug('[OfficialAccount] The model has been updated [Model] %s' % self.__dict__)
